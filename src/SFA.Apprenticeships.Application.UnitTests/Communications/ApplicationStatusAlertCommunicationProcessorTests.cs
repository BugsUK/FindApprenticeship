namespace SFA.Apprenticeships.Application.UnitTests.Communications
{
    ...
    {
        [Test]
        public void OneEmailPerCandidate()
        {
            ...
            var processor = new CommunicationProcessorBuilder().With(applicationStatusAlertRepository).With(expiringDraftRepository).With(candidateReadRepository).With(messageBus).Build();
            var batchId = Guid.NewGuid();

            processor.SendDailyCommunications(batchId);

            messageBus.Verify(mb => mb.PublishMessage(It.IsAny<CommunicationRequest>()), Times.Exactly(3));
        }

        [Test]
        public void AllowEmailsFalseShouldDeleteAlerts()
        {
            var applicationStatusAlertRepository = new Mock<IApplicationStatusAlertRepository>();
            applicationStatusAlertRepository.Setup(x => x.GetCandidatesDailyDigest()).Returns(GetAlertCandidatesDailyDigest(2, 2));
            var candidateReadRepository = new Mock<ICandidateReadRepository>();
            var candidate = new CandidateBuilder(Guid.NewGuid()).AllowEmail(false).Build();
            candidateReadRepository.Setup(x => x.Get(It.IsAny<Guid>())).Returns(candidate);
            var processor = new CommunicationProcessorBuilder().With(applicationStatusAlertRepository).With(candidateReadRepository).Build();
            var batchId = Guid.NewGuid();
