CREATE PROCEDURE [dbo].[uspRecruitmentAgencySelectTaskSummary]
	@ProviderId int,
...
BEGIN
	...
	SELECT @FilledVacToComplete = Count(*)
		FROM 
			(SELECT dbo.Vacancy.VacancyId, dbo.Vacancy.NumberofPositions As NumberOfPositions,
				...
				WHERE dbo.[ProviderSite].ProviderSiteID = @providerID AND dbo.Vacancy.VacancyManagerID = @RecruitmentAgencyId AND (dbo.Vacancy.VacancyStatusId <>
					(SELECT VacancyStatusTypeId	FROM dbo.VacancyStatusType AS VacancyStatusType_1 WHERE CodeName = N'Com')
				)
			) X
		WHERE (X.NumberOfPositions - X.PositionsFilled) = 0

	-- Vacancies with Missing ILR Details
	SELECT @MissingILRDetails = 0-- Unfilled Vacancies past their closing date
	...
	SELECT @VacDescRework = COUNT(1)--select *
		...
		AND [vacancy].VacancyManagerID = @RecruitmentAgencyId
	...
	select @CandidateWithdrawalSearchCount = count(app.ApplicationId)
		...
		where 
		v.VacancyManagerID = @RecruitmentAgencyId
		AND app.ApplicationStatusTypeId=4 
