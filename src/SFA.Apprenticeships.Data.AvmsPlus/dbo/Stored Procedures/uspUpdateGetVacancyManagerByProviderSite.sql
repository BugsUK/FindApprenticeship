CREATE PROCEDURE [dbo].[uspUpdateGetVacancyManagerByProviderSite]
...
BEGIN
    ...
    WHERE  (
            ...
            (V.VacancyLocationTypeId != 2 OR V.VacancyLocationTypeId IS NULL) AND V.VacancyManagerID = @OldProviderSiteId
	...
	OR      (
            ...
            MasterVacancyId IN  (SELECT VacancyId 
                                 FROM   Vacancy V
                                 WHERE V.VacancyLocationTypeId = 2 AND V.VacancyManagerID = @OldProviderSiteId
                                 AND (V.VacancyStatusId = 1
                                      OR   V.VacancyStatusId = 2
                                      OR   V.VacancyStatusId = 3
                                      OR   V.VacancyStatusId = 5
	...
	/*UPDATE STATEMENT */
	UPDATE V
	SET    V.VacancyManagerID = @NewProviderSiteId
	FROM   Vacancy V
           INNER JOIN VacancyStatusType VST ON V.VacancyStatusId = VST.VacancyStatusTypeId
    WHERE  (
            /* SINGLE SITE VACANCIES */
