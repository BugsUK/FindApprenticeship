CREATE PROCEDURE [dbo].[uspTransferVacancySearch]  
...
@trainingProviderFilterSiteId  AS NVARCHAR(400) = ''
AS  
BEGIN  
    SET NOCOUNT ON  
...
BEGIN TRY  
 ...
 DECLARE @employerName2          AS NVARCHAR(255)  
  ...
  DECLARE @psFilterSiteIds TABLE  
    ...
    -- Need to tack a delimiter onto the end of the input string if one doesn?t exist  
    SET @workingIds = @trainingProviderFilterSiteId   
    IF RIGHT(RTRIM(@workingIds),1) <> ','  
        SET @workingIds = @workingIds  + ','  
    ...
    -- You have a piece of data, so insert it, print it, do whatever you want to with it.  
        INSERT INTO @psFilterSiteIds VALUES ( @piece )  
                ...
                FROM  
                    (  						
						...
						* FROM @ResultsStandardVacancy rsv
						WHERE
						( rsv.ApprenticeshipFrameworkId = @apprenticeFrameworkId OR @apprenticeFrameworkId = -1 OR ( @apprenticeFrameworkId = 0 AND rsv.ApprenticeshipFrameworkId IS NULL ) )  AND
						( rsv.ContractOwnerId = @contractOwnerId OR @contractOwnerId = -1 OR ( @contractOwnerId = 0 AND rsv.ContractOwnerId IS NULL)  ) AND
						( rsv.VacancyManagerId = @vacancyManagerId OR @vacancyManagerId = -1 OR ( @vacancyManagerId = 0 AND rsv.VacancyManagerId IS NULL  ) ) AND
                        ( rsv.DelivererID = @delivererId OR @delivererId = -1 OR ( @delivererId = 0 AND rsv.DelivererID IS NULL ) )  AND
                        ( rsv.OriginalContractOwnerId = @originalContractOwnerId OR @originalContractOwnerId = -1  OR ( @originalContractOwnerId = 0 AND rsv.OriginalContractOwnerId IS NULL ) ) AND
						( rsv.EmployerTradingName LIKE '%' + @employerName + '%' OR rsv.EmployerTradingName LIKE '%' + @employerName2 + '%' OR @employerName = '')	AND 
						( rsv.ManagingAreaId = @regionId or @regionId = -1 or (@regionId = 0 AND rsv.ManagingAreaId IS NULL) ) AND 
						( rsv.TrainingProviderId IN ( SELECT psId FROM @psFilterSiteIds ) OR @trainingProviderFilterSiteId = '' )
						...
						* from @ResultsNatMultVacancy rnmv
						WHERE
						( rnmv.ApprenticeshipFrameworkId = @apprenticeFrameworkId OR @apprenticeFrameworkId = -1 OR ( @apprenticeFrameworkId = 0 AND rnmv.ApprenticeshipFrameworkId IS NULL ))  AND
						( rnmv.ContractOwnerId = @contractOwnerId OR @contractOwnerId = -1 OR ( @contractOwnerId = 0 AND rnmv.ContractOwnerId IS NULL)  ) AND
						( rnmv.VacancyManagerId = @vacancyManagerId OR @vacancyManagerId = -1 OR ( @vacancyManagerId = 0 AND rnmv.VacancyManagerId IS NULL  ) ) AND
                        ( rnmv.DelivererID = @delivererId OR @delivererId = -1 OR ( @delivererId = 0 AND rnmv.DelivererID IS NULL ) ) AND
                        ( rnmv.OriginalContractOwnerId = @originalContractOwnerId OR @originalContractOwnerId = -1 OR ( @originalContractOwnerId = 0 AND rnmv.OriginalContractOwnerId IS NULL ) ) AND
                        ( rnmv.EmployerTradingName LIKE '%' + @employerName + '%' OR rnmv.EmployerTradingName LIKE '%' + @employerName2 + '%' OR @employerName = '') AND
                        ( rnmv.ManagingAreaId = @regionId or @regionId = -1 or (@regionId = 0 AND rnmv.ManagingAreaId IS NULL))  AND 
						( rnmv.TrainingProviderId IN ( SELECT psId FROM @psFilterSiteIds ) OR @trainingProviderFilterSiteId = '' )       
